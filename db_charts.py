...
def scatter_plots(sql_result_list, file_name):
    ...
    df.rename(columns={0: 'ROWNUM', 1: 'SQL_FINISH_TIME', 2: 'QUERY_TIME', 3: 'EXEC_CLIENT', 4: 'SQL_TEXT', 5: 'OPTYPE'},
              inplace=True)
    df = df.sort_values(['QUERY_TIME'], ascending=[1])
    # x是横线坐标数据来源，y是纵向坐标数据来源，匹配上面df重命名的列，color是用于对不同操作类型进行分类点击，log_y可以记录详细的信息
    # hover_data是悬停到浮点展示的各类信息
    fig = px.scatter(df, x="SQL_FINISH_TIME", y="QUERY_TIME", color="OPTYPE", log_y=True,
                     hover_data={'SQL_FINISH_TIME': '|%H:%M:%S.%L',
                                 'EXEC_CLIENT': True,
                                 'SQL_TEXT': True,
                                 })
    fig.update_traces(marker_size=5)  # 散点大小
    fig.update_layout(
        title={'text': "SQL执行情况分布"},
        title_font_color="red",  # 散点图文字，标题等格式设置
        xaxis=dict(title='时间范围'),  # x轴坐标名称
        yaxis=dict(title_text="SQL执行耗时<b>(秒)</b>", titlefont=dict(color="#ff7f0e"), ),  # y轴坐标名称
    ...
    fig.update_yaxes(ticklabelposition="outside top", ticksuffix="毫秒", showgrid=True, gridcolor='LightPink'
                     , zeroline=True, zerolinewidth=2, zerolinecolor='LightPink', tickformat=".3f")
